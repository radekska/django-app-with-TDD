...
MANAGE_DOT_PY = 'heroku run python3 manage.py'
...
def reset_database(staging_server):
    ...
    os.system(f'{MANAGE_DOT_PY} makemigrations --app={staging_server}')
    os.system(f'{MANAGE_DOT_PY} migrate --app={staging_server}')
...
def create_session_on_server(staging_server, email):
    session_key = os.popen(f'{MANAGE_DOT_PY} create_session {email} --app={staging_server}').read()
