...
class NewListTest(TestCase):
    ...
    def test_lists_owner_is_save_if_authentiacted(self, mockItemFormClass, mockListClass):
        user = User.objects.create(email='a@b.com')
        self.client.force_login(user)
        mock_list = mockListClass.return_value
        mock_list.get_absolute_url.return_value = '/lists/1'

        def check_owner_assigned():
            self.assertEqual(mock_list.owner, user)

        mock_list.save.side_effect = check_owner_assigned
        self.client.post(path='/lists/new', data={
            'text': 'new item'
        })
...
class MyListsTest(TestCase):
    ...
    def test_passes_correct_owner_to_template(self):
        ...
        response = self.client.get(path='/lists/users/correct@owner.com/')
